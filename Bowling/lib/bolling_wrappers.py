...
class BollingWrapFrame(gym.ObservationWrapper):
    def __init__(self, env, width=84, height=40, grayscale=True, box=(0,100,160,175), dict_space_key=None):
        """
        Warp frames to 84x84 as done in the Nature paper and later work.
        If the environment uses dictionary observations, `dict_space_key` can be specified which indicates which
        observation should be warped.
...
def make_bolling(env_id, noop=True, maxskip=True,
...
def wrap_bolling(env,frame_stack=True, scale=True, pytorch_img=True,
                  frame_stack_count=4, skip_firereset=False):
    """Configure environment for DeepMind-style Atari.
    """
    if 'FIRE' in env.unwrapped.get_action_meanings():

        ...
        while True:
            ...
            if new_rewards:
                if reward_tracker.reward(new_rewards[0], frame_idx, selector.epsilon):
                    tgt_net.sync()
                    torch.save(tgt_net.target_model.state_dict(), params['run_name']+'_target_net.dat')
                    break
            ...
            loss_v = common.calc_loss_dqn(batch, net, tgt_net.target_model, gamma=params['gamma']**4, device=device)
            loss_v.backward()
            optimizer.step()

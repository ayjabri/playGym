...
class A3CNet(nn.Module):
    def __init__(self, obs_size, act_size):
        ...
        self.base = nn.Sequential(nn.Linear(obs_size, 512),
                                nn.ReLU(),
                                nn.Linear(512, 256),
                                nn.ReLU())
...
def data_func(net, device, train_queue):
    ...
    exp_source = ptan.experience.ExperienceSourceFirstLast(envs, agent, GAMMA,\
                                                           steps_count= STEPS)
    mini_batch = []
    ...
    try:
        while True:
            ...
            if step_idx % TEST_ITER == 0:
                rewards, steps = test_net(net,env)
                print(f'epoch:{epoch:6} mean:{rewards:7.2f}, policy_loss:{policy_loss:7.2f},value_loss:{value_loss:7.2f}, entropy_loss:{entropy_loss:7.2f}')
                if rewards > top_reward:
                    top_reward = rewards
                    print('New top rewards!')
                    torch.save(net.state_dict(),'lunar_a3c.dat')
                if rewards > SOLVE_BOUND:
                    print('Solved!')
                    break

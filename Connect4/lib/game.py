...
class Connect4(object):
    ...
    def decode_binary(self, state_int):
        ...
        bits = self.int_to_bits(state_int, bits=self.space + self.cols*self.bits_in_len)
        res = []
    ...
    def move(self, state_int, col, player):
        ...
        if not won:
            won = self._check_won(field, col, 0) or self._check_won(field, col, 1) or self._check_won(field, col, -1)

...
if __name__=='__main__':
    ...
    for exp in exp_source:
        ...
        if new_reward:
            ...
            if delta_time > 2:
                print(f'Epoch:{epoch:3}, Loss: {loss_v:3.2f}, Reward:{new_reward[0]:6.2f}, Mean Rewards:{mean:6.2f}')
                print_time = time()
            if mean > SOLVE_REWARD:
                done_training = datetime.now()
                training_time = timedelta(seconds=(done_training-start_training).seconds)
                print(f'Epoch:{epoch:3}, Loss: {loss_v:6.2f}, Reward:{new_reward[0]:6.2f}, Mean Rewards:{mean:6.2f}')
                print(f'Solved in {training_time} seconds after seeling {len(total_rewards)} episodes!')
                break
    ...
    if args.save: torch.save(net.state_dict(), 'trainedModels/actorCritic.dat')

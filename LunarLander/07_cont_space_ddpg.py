...
def unpack_dqn(batch, device='cpu'):
    ...
    actions_v = torch.FloatTensor(np.array(actions, copy=False))
...
if __name__=='__main__':
    ...
    actor_net = DDPGActorNet(obs_size, act_size)
    tgt_actor_net = ptan.agent.TargetNet(actor_net)
    critic_net = DDPGCriticNet(obs_size, act_size)
    ...
    while True:
        ...
        if new_reward:
            ...
            if mean>BOUND:
                ...
                break
            if time()-pt > 2:
                print(f'{frame_idx}: episode:{episode}, mean:{mean:7.2f}, loss:{actor_loss:7.2f}')
                pt = time()
